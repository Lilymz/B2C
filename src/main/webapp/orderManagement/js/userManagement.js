$(function(){
	...
	//默认加载所有用户
	$.getJSON('/o2o/superadmin/getAllUser',function(data){
		if(data.success){
			$('.UserName').html('你好,'+data.adminInfo.adminName);
			tdHtml='';
			data.userInfos.map(function(item,index){
	...
	$('.Color1Btn ').bind('click',function(){
		...
		if(userName==""){
			...
			$('tbody').html('');
			$.ajax({
				url:'/o2o/superadmin/userSearchByName',
				type:'POST',
	...
	$('tbody').delegate('.PublicTableBtnIcon','click',function(){
		...
		$(".Js_edit").click(function () {
			   ...
			   $('.publicf_btn1').bind('click',function(){	
					//获取输入状态值
					var userEnableStatus=$('.f_p_input').val();
					var Formvalue=new FormData();
					Formvalue.append('currentId',currentId);
					Formvalue.append('userEnableStatus',userEnableStatus);
					$.ajax({
							url:'/o2o/superadmin/updateUserByEnableStatus',
							type:'POST',
							data:Formvalue,
							dataType:'json',
							contentType:false,
		...
		$(".Js_delete").click(function () {
		       ...
		       $('.f_pormatBtn1 ').bind('click',function(){
		    	   $.ajax({
						url:'/o2o/superadmin/deleteUserByuserId',
						type:'POST',
